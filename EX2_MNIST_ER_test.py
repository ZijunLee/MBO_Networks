...
W_boyd = gl.weightmatrix.knn(order_raw_data_HU, 10)
degree_W_boyd = np.array(np.sum(W_boyd, axis=-1)).flatten()


# Compute the eigenvalues and eigenvectors of L_mix_sym and L_mix_rw
eig_val_MMBO_sym, eig_vec_MMBO_sym, eig_val_MMBO_rw, eig_vec_MMBO_rw, order_raw_data_MMBO, index_MMBO, time_eig_l_mix_sym, time_eig_l_mix_rw = nystrom_QR_l_mix_sym_rw_ER_null(Z_training, num_nystrom=1000, tau = tau)
...
for _ in range(20):
    ...
    #u_init_HU_sup = u_init
    #u_init_HU_sup[[Rs],:] = gt_HU_vec[[Rs],:]

    #u_init_LWP_sup = u_init
    ...
    ARI_boyd_l_sym = adjusted_rand_score(u_boyd_sym_label, gt_labels_boyd)
    purify_boyd_l_sym = purity_score(gt_labels_boyd, u_boyd_sym_label)
    inverse_purify_boyd_l_sym = inverse_purity_score(gt_labels_boyd, u_boyd_sym_label)
    NMI_boyd_l_sym = normalized_mutual_info_score(gt_labels_boyd, u_boyd_sym_label)
    ...
    ARI_boyd_l_rw = adjusted_rand_score(u_boyd_rw_label, gt_labels_boyd)
    purify_boyd_l_rw = purity_score(gt_labels_boyd, u_boyd_rw_label)
    inverse_purify_boyd_l_rw = inverse_purity_score(gt_labels_boyd, u_boyd_rw_label)
    NMI_boyd_l_rw = normalized_mutual_info_score(gt_labels_boyd, u_boyd_rw_label)
...
print('average_ER_modularity_boyd_rw: ', average_ER_modularity_boyd_rw)
