...
#feature_map_nystroem = Nystroem(gamma=gamma_002,random_state=1,n_components=50)
...
D_hu_arpack, V_hu_arpack = eigsh(
    sym_graph_lap,
    k=m,
    sigma=0,
#    v0=np.ones((laplacian_mix.shape[0], 1)),
    which='LA')
time_eig_l_sym_arpack = time.time() - start_time_eigendecomposition_l_sym
print("compute eigenvalues and eigenvectors of L_{F_sym} for HU's method:-- %.3f seconds --" % (time_eig_l_sym_arpack))
#print('EPSType is krylovschur')
#D_hu = eigenpair_hu[0]
#V_hu = eigenpair_hu[1]
print('ARPACK D_sign shape: ', D_hu_arpack)
print('ARPACK V_sign shape: ', V_hu_arpack.shape)


print('Using nystrom extension')
start_time_eigendecomposition = time.time()
eigenvalues_1, eigenvectors_1 = nystrom_new(train_data, num_nystrom=500, gamma=gamma_002)
D_hu_nys = np.squeeze(eigenvalues_1[:m])
V_hu_nys = eigenvectors_1[:,:m]
time_eig_l_sym_nys = time.time() - start_time_eigendecomposition
print("compute eigendecomposition:-- %.3f seconds --" % (time_eig_l_sym_nys))
print('nystrom D_sign shape: ', D_hu_nys)
print('nystrom V_sign shape: ', V_hu_nys.shape)
...
print('Using eigenpairs from ARPACK')
# Test HU original MBO with symmetric normalized L_F
start_time_hu_original = time.time()
u_hu_vector, num_iter_HU = mbo_modularity_hu_original(num_nodes, num_communities, m, degree, dt_inner, u_init,
                             D_hu_arpack, V_hu_arpack, tol,inner_step_count) 
time_hu_mbo = time.time() - start_time_hu_original
print("HU original MBO:-- %.3f seconds --" % (time_eig_l_sym_arpack + time_initialize_u + time_hu_mbo))
...
print('Using eigenpairs from nystrom extension')
start_time_hu_original = time.time()
u_hu_vector, num_iter_HU = mbo_modularity_hu_original(num_nodes, num_communities, m, degree, dt_inner, u_init,
                             D_hu_nys, V_hu_nys, tol,inner_step_count) 
time_hu_mbo = time.time() - start_time_hu_original
print("HU original MBO:-- %.3f seconds --" % (time_eig_l_sym_nys + time_initialize_u + time_hu_mbo))
print('HU original MBO the num_iteration: ', num_iter_HU)
...
#u_1_nor_Lf_Qh_individual_1,num_repeat_1_nor_Lf_Qh_1 = mbo_modularity_1(num_nodes,num_communities, m_1, dt_inner, u_init, 
#                                                 D_mmbo, V_mmbo, tol, gamma_1)
...
#                                        D_mmbo, V_mmbo, dt_inner, tol,target_size, inner_step_count)
